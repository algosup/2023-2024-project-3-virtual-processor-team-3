...
// ◊⏛⎯=⎯=⎯=⎯=⎯=⎯=⎯=⎯=⎯⏛: ⎩°⁍ PARSE DATA LINE ⁌°⎭ :⏛⎯=⎯=⎯=⎯=⎯=⎯=⎯=⎯=⎯⏛◊
    ...
    int ParseLineData(char *line, DataSection_t *dataSection, SymbolTableNode_t **head, int *currentLine) {
        // ○⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎱Syntax Checking Preparation⎰⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯○
            ...
            int currentAddress = dataSection->bytesCount;
        ...
        // ○⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎱Discarding Comments⎰⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯○
            ...
            if (i > 0) {
                while (lineCopy[i]) {
                    lineCopy[j++] = lineCopy[i++];
                }
                lineCopy[j] = '\0'; // Null-terminate the shifted string
            }
            ...
            // Check if the lineCopy is empty after removing comments and trimming whitespace
            if (strlen(lineCopy) == 0) {

                // If the lineCopy is empty, skip the printing process and continue with next lineCopy
                return 0; 
            }          

        // ○⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎱Syntax Checking⎰⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯○
            // Check if the line doesn't start with a letter
            ...
            // Check if declaration is missing the colon operator
            ...
            // Check if the first non-space character after the colon is not the expected dot
            if (*directiveCheck != '.') {
                    ...
                    if (strcmp(directiveName, "byte") == 0) {
                        ...
                        if (byteCount > 0 && byteCount <= MAX_BYTE_ARRAY_DECLR ) {
                            ...
                            if (MAX_DATA_MEMORY - dataSection->bytesCount >= byteCount){
                                ...
                                insertNode(head, label, currentAddress);
